...
function findLongestWord(sentence) {
  ...
  const longestWord = wordsArr.reduce((longest, word) => {
...
function extractNameFromTemplate(value) {
  const startIdx = value.indexOf('Hello, ') + 'Hello, '.length;
...
function getCardId(value) {
  ...
  const rank = value.slice(0, -1); // Витягаємо ранг карти (усе, крім останнього символу)
  const suit = value.slice(-1); // Витягаємо масть карти (останній символ)

  const rankIndex = ranks.indexOf(rank);
  const suitIndex = suits.indexOf(suit);

  if (rankIndex !== -1 && suitIndex !== -1) {
    // Якщо ранг і масть коректні, то обчислюємо і повертаємо індекс карти
